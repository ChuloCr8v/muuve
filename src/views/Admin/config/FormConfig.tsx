...
import { projectForm } from "../../../component/TableItems/data/FormItem";
  ...
  const items: TabsProps["items"] = [
    {
      ...
      children: <FormSettings selectedForm={selectedForm} setSelectedForm={setSelectedElements} selectedElements={selectedElements} />,
    },
  ];
          ...
          {projectForm.map((formList, index) => (
            <p
              ...
              className={twMerge(
                ...
                selectedForm.label === formList.label ? 'text-primary font-semibold' : ''
              )}
            >
              {formList.label}
            </p>
  ...
  return (
    <main className="flex w-full p-[24px] h-[100%] bg-[#f6f6f6] overflow-hidden">
        ...
        <section className="w-[30%] h-full rounded-md border-[1px] border-[#D9DADC] p-[16px] bg-white overflow-hidden ml-4"> 
          ...
          <Tabs defaultActiveKey="1" className="w-full" items={options} onChange={onChange} />
        </section>
</main>
  );
}
