      ...
      <section className="w-[40%] overflow-hidden">
        ...
        <section className="h-[calc(100vh-90px)] overflow-y-auto">
          {maintenance.map((data, index) => (
            <div
              key={index}
              onClick={() => handleFile(data)}
              className={twMerge(
                "flex justify-between px-3 items-center py-1 border-b-[1.6px]",
                accpet && "bg-green-50",
                reject && "bg-red-50"
              )}
            >
              <div className={twMerge()}>
                <p className="text-[12px] leading-0 font-semibold">
                  {data.name}
                </p>
              ...
              <div className="space-x-2">
                <Tooltip title="Accept" color="#379D51" placement="left">
                  <CheckOutlined
                    onClick={() => setAccpet(true)}
                    className={twMerge(
                      "text-green-600 font-semibold p-2",
                      accpet && "bg-green-600 text-white rounded-[5px]"
                    )}
                  />
                </Tooltip>
                <Tooltip title="Reject" color="#E44136" placement="right">
                  <CloseOutlined
                    ...
                    className={twMerge(
                      "text-green-600 font-semibold p-2",
                      reject && "bg-red-600 text-white rounded-[5px]"
                    )}
                  />
                </Tooltip>
              </div>
