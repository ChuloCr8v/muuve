...
export default function ReportDetails() {
  ...
  const column = [
    ...
    },
    {
      ...
      render: (_: any, record: any) => (
        <TableRowData mainText={record?.name} tagText={record.descriptions} />
      ),
    },
    {
      title: "Availability",
    ...
    },
    {
      ...
      render: (_: any, record: any) => (
        <p>
          {record.issue}{" "}
          <span className="cursor-pointer text-[13px] text-[#0A95CC] font-semibold">
            {record.action}
          </span>
        </p>
      ),
      ...
      render: (text: string) => (
        <Tag
          className={twMerge(
            ...
            "rounded-2xl text-[12px] font-semibold"
          )}
        >
          {text}
        </Tag>
  ...
  return (
    <div className="space-y-[16px] p-8">
      ...
      <TableComponent
        onRow={(record: Array<{}>) => ({
          onClick: () => handleOpenIssue(record),
        })}
        columns={column}
        dataSource={details}
        scroll={500}
