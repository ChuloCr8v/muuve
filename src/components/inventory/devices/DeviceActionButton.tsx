...
export default function DeviceAction(props: Prop) {
  ...
  const LabelItem = (props: {
    ...
    return (
      <div
        className={twMerge(
          "font-[400] text-[14px] flex space-x-2",
          props.style
        )}
      >
    ...
    <div>
      ...
      <ActionPopup
        ...
        icon={
          <div
            className={twMerge(
              "text-5xl w-[60px] flex h-[60px] items-center justify-center rounded-full  p-2",
          ...
          >
            {actionType === "Assign Device" ? (
              <UserAddOutlined className="text-[30px]" />
            ) : actionType === "Report Fault" ? (
              <CloseCircleOutlined className="text-[30px] text-red-600 " />
            ) : (
              <DeleteOutlined className="text-[30px] text-red-600 " />
            )}
          </div>
        }
        sendButtonStyle={`${
      ...
      >
        {actionType === "Assign Device" ? (
          <AssignDevice selectedRow={selectedRow} />
        ) : actionType === "Report Fault" ? (
          <DeviceFault selectedRow={selectedRow} />
        ) : (
