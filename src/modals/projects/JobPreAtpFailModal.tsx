...
import { useFailPreAtpMutation } from "../../api/project.api";
...
export const JobPreAtpFailModal = ({ project }: Props) => {
  const [confirm, setConfirm] = useState(false);

  const [form] = Form.useForm();
  const { closeModal } = usePopup();

  const [fail, { isLoading }] = useFailPreAtpMutation();
  ...
  const submit = async () => {
    ...
    fail(data)
      .unwrap()
      .then(() => {
        message.success("Pre-ATP Failed");
        closeModal();
      })
      .catch(toastApiError);
  };
